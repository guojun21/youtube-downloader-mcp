...
export function startBackgroundVideoDownloadTask({
  ...
  // Why: the throttle in persistProgressPatchToDisk may leave a pending patch
  // unflushed if no new output arrives within the 1-second window. This interval
  // ensures accumulated progress is written to disk at most every 2 seconds.
  const flushInterval = setInterval(() => {
    if (pendingProgressPatch) {
      const finalizedPatch = pendingProgressPatch;
      pendingProgressPatch = null;
      lastDiskPersistTimestamp = Date.now();
      updateExistingTaskRecord(taskId, finalizedPatch);
    }
  }, 2000);
