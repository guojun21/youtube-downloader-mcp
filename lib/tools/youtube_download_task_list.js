...
export const definition = {
  ...
  description:
    "View history of all YouTube tasks (downloads, subtitle fetches, and transcriptions). Shows date, video ID, type, output path, status, and percentage for each task.",
  inputSchema: {
    type: "object",
    properties: {
      type: {
        type: "string",
        description:
          "Filter by task type: 'download' or 'subtitle' or 'transcription'. Omit to show all.",
      },
      status: {
        type: "string",
...
export async function handler(args = {}) {
  ...
  const taskSummaries = paginatedTaskRecords.map((task) => {
    const taskType = task.type || "download";
    const baseSummary = {
      id: task.id,
      type: taskType,
    ...
    if (taskType === "download") {
      ...
      baseSummary.video_file_path = task.video_file_path || null;
      baseSummary.transcript_path = task.transcript_path || null;
      baseSummary.language = task.language || null;
      baseSummary.detected_language = task.detected_language || null;
      baseSummary.model = task.model || null;
      baseSummary.elapsed_seconds = task.elapsed_seconds || null;
    }
